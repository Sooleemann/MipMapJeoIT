...
{
    public class TkgmEnglishController : Controller
    {
        private readonly ILogger<HomeController> _logger;
        private readonly IWebHostEnvironment hostEnvironment;

        public TkgmEnglishController(ILogger<HomeController> logger, IWebHostEnvironment hostEnvironment)
        {
            _logger = logger;
            ...
            try
            {
                ...
                using (HttpClient client = new HttpClient())
                {
                    using (HttpResponseMessage res = await client.SendAsync(req))
                    {
                        ...
                        {
                            return Ok(new { success = false, msg = "Günlük sorgu limitini aştınız" });
                        }
                    }
                }
                ...
                {
                    return Ok(new { success = false, msg = "Geri dönüş değeri alınamadı." });
                }
                else
                {
                    if (data.Contains("Günlük sorgu limitini aştınız"))
                        return Ok(new { success = false, msg = "Günlük sorgu limitini aştınız" });
                }
                ...
                if (obj != null)
                {
                    ...
                    return Ok(new { success = false, msg = "Sorgu sonucu boş gelmiştir." });
            }
            catch (Exception e)
            {
